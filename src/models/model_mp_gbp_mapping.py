...
class MPGBPMapping(ModelBase):
    ...
    def __set_point_name(self):
        ...
        response: Response = gw_request(api=f'/ps/api/modbus/points/uuid/{self.point_uuid}')
    ...
    def __set_point_uuid(self):
        ...
        response: Response = gw_request(api=f'/ps/api/modbus/points/name/{network_name}/{device_name}/{point_name}')
        if response.status_code != 200:
    ...
    def __set_mapped_point_uuid(self):
        ...
        if self.type in (MapType.GENERIC.name, MapType.GENERIC):
            ...
            response: Response = gw_request(
                api=f'/ps/api/generic/points/name/{network_name}/{device_name}/{point_name}')
        elif self.type in (MapType.BACNET.name, MapType.BACNET):
